    ...
    return (
        <Backdrop style={{display: 'block', overflow: 'scroll', backgroundColor: 'rgba(0, 0, 0, 0.86)'}} open={open} sx={{zIndex: 1}}>
            ...
            <div style={{marginTop: isMobile ? '20vw' : '12%'}}>
                {loading ? <CircularProgress style={{marginTop: '10%'}} color='darkGreen' size={200} /> : <div style={{display: 'flex', justifyContent: 'space-evenly', flexWrap: 'wrap'}}>{mapDataToCards(cards)}</div>}
            </div>
